...
Le script `start:prod` conserve temporairement `prisma db push`. Une étape de pré-déploiement idempotente transfère d’abord les anciennes données de motifs vers leur table de liaison, afin que `db push` puisse terminer l’évolution sans `--accept-data-loss`. Il ne devra être remplacé par `prisma migrate deploy` qu’après le baselining vérifié de chaque base existante. Cette séparation évite qu’un simple redéploiement tente d’appliquer la migration initiale sur une base déjà remplie.
